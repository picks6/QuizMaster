...
const resolvers = {
	Query: {
		...
		//deck category
		deckCategory: async (parent, { categoryID }) => {
			// category: [ID]
	...
	Mutation: {
		...
		updateCategory: async (parent, args, context) => {
			// args: { categoryId: ID!, category: String! }
			if (context.deck) {
				return await Category.findByIdAndUpdate(context.category._id, args, {
					new: true,
				});
			}
		...
		updateDeck: async (parent, args, context) => {
			...
			if (context.deck) {
				return await Deck.findByIdAndUpdate(context.deck._id, args, {
					new: true,
				});
			}
			if (args.deckId) {
		...
		updateCard: async (parent, args, context) => {
			...
			if (context.deck) {
				// TODO: Reconfigure to query Deck for Cards to update
				return await Card.findByIdAndUpdate(context.card._id, args, {
					new: true,
				});
			}
			if (args.cardId) {
